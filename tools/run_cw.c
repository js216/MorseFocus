...
{
    fprintf(stderr,
        "Usage: %s [-f <file>] [-1 <speed1>] [-2 <speed2>] [-t <freq>] [-a <amp>] [-d <delay>]\n"
        "  -f <file>    Read input string from file (default stdin)\n"
        "  -1 <speed1>  Speed1 in WPM (1..500), default 25\n"
        "  -2 <speed2>  Speed2 in WPM (1..500), default 25\n"
        "  -t <freq>    Tone frequency Hz (60..10000), default 700\n"
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-f") == 0) {
            ...
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -1 requires a speed value\n");
                print_usage(argv[0]);
                return -1;
            }
            speed1 = atof(argv[++i]);
            if (check_float_range(speed1, 1.0f, 500.0f, "speed1") < 0) return -1;
        } else if (strcmp(argv[i], "-2") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -2 requires a speed value\n");
                print_usage(argv[0]);
                return -1;
            }
            speed2 = atof(argv[++i]);
            if (check_float_range(speed2, 1.0f, 500.0f, "speed2") < 0) return -1;
        } else if (strcmp(argv[i], "-t") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -t requires a frequency value\n");
                print_usage(argv[0]);
                return -1;
            }
            freq = atof(argv[++i]);
            if (check_float_range(freq, 60.0f, 10000.0f, "frequency") < 0) return -1;
        } else if (strcmp(argv[i], "-a") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -a requires an amplitude value\n");
                print_usage(argv[0]);
                return -1;
            }
            amp = atof(argv[++i]);
            if (check_float_range(amp, 0.0f, 1.0f, "amplitude") < 0) return -1;
        } else if (strcmp(argv[i], "-d") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -d requires a delay value\n");
                print_usage(argv[0]);
                return -1;
            }
            delay = atof(argv[++i]);
            if (check_float_range(delay, 0.0f, 60.0f, "delay") < 0) return -1;
        } else {
